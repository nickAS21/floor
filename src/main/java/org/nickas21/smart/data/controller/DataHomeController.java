...
public class DataHomeController {
    ...
    public DataHomeController(SettingsService settingsService, UserService userService, TuyaDeviceService tuyaDeviceService, TuyaConnectionProperties tuyaConnectionProperties) {
    ...
    @GetMapping("/golego")
    public Mono<ResponseEntity<String[]>> getInfoHomeGolego(@RequestHeader(required = false, value = "Authorization") String token) {
    ...
    @GetMapping("/dacha")
    public Mono<ResponseEntity<String[]>> getInfoHomeDacha(@RequestHeader(required = false, value = "Authorization") String token) {
        return userService.validateToken(token)
                .flatMap(isValid -> {
                    if (isValid) {
                        if (this.tuyaDeviceService.devices != null) {
